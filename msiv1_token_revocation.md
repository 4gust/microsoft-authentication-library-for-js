...
2. Allow Resource Providers (RPs) to perform token revocation.
3. Provide telemetry signals for eSTS to differentiate which apps are CAE enlightened.
...
var result = await client.AcquireTokenAsync(
    resource,
    options => options.WithClaims("eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6Ik5UZG1aak00")
);
```
...
For the token revocation to work properly, implementations must correctly integrate with each library's existing cache mechanism:

##### MSAL .NET
```csharp
// Uses the ITokenCache abstraction
// Existing code pattern for token cache integration:
var tokenCache = app.UserTokenCache.RecordAccess(args);
```

##### MSAL Python
```python
# Uses the TokenCache class
# Example of token cache integration:
token_cache = msal.SerializableTokenCache()
client = ManagedIdentityClient(token_cache=token_cache)
```

##### MSAL JavaScript
```javascript
// Uses the TokenCache and PersistenceCachePlugin
// Example of token cache integration:
const cachePlugin = new PersistenceCachePlugin(cacheOptions);
const client = new ManagedIdentityClient({
  cache: cachePlugin
});
```
...
##### MSAL Java
```java
// Uses the ITokenCacheAccessAspect interface
// Example of token cache integration:
ITokenCacheAccessAspect persistenceAspect = new TokenPersistenceAspect();
ManagedIdentityClient client = new ManagedIdentityClientBuilder()
...
##### MSAL Go
```go
// Uses storage.Manager for token cache
// Example of token cache integration:
client, err := managedidentity.New(
    managedidentity.SystemAssigned(),
    managedidentity.WithStorage(customStorage)
)
```
...
- When claims and client capabilities are provided, MSAL must include `token_sha256_to_refresh` parameter in requests
